...
const SingleCor = ({ item }) => {
    ...
    const handleAddToCarts= item =>{
        ...
        if(user && user.email){
            ...
            fetch('https://yoga-time.vercel.app/carts',{
                method: 'POST',
                headers:{
    ...
    return (
        <div className="">
            <div className="card card-side grid lg:grid-cols-2 border rounded-lg border-cyan-400 m-2 mt-5  ">
               
                <figure><img className="" src={image} alt="" /></figure>
              
                <div className="card-body text-white bg-black rounded  border-cyan-400 ps-9 font-black text-l">
                    <h2 className="font-black text-xl">Name : {name}</h2>
                    <p >Instructor : {instructor}</p>
                    <p>Seat : {availableSeats}</p>
                    <p>Price : {price}</p>
                    <div className="card-actions justify-end">
                    <button onClick={()=>handleAddToCarts(item)} className="btn btn-outline text-cyan-400">Select</button>
                    </div>
                </div>
            </div>
        </div>
